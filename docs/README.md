# docs

- [ICL.md](ICL.md) the three assistance levels, code links and final results.
- `INTERFACE.md` the frozen answer contract and record schema. Section 7 is
  what `ecpm_parser.py` implements.
- `EXPLORE_AGENT.md` the agentic exploration arm.
- `PAPER_SPINE.md` proposed claim, hypotheses as a pre-registration,
  readiness audit, budget. For discussion, not settled.
- `SEED_SELECTION.md` the three eligibility criteria, the 23-seed run set,
  and the nine restraint cases. Replaces the seed paragraph in the
  methodology document.
- `DIAGRAMS.md` paste-ready mermaid blocks and the figure palette.
- `diagrams/` rendered SVGs.
- `source/` the team research documents this repository was written against,
  kept so the numbers can be traced to what was claimed at the time.
